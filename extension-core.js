...
extension.events.mouse.mousemove = function (event) {
    ...
    document.dispatchEvent(new CustomEvent('ui-mousemove', {
        detail: {
            event
        }
    }));
...
extension.events.mouse.mousedown = function (event) {
    if (extension.ui.hover(extension.ui.surface)) {
        event.preventDefault();
        event.stopPropagation();

        if (extension.storage.items.hidden !== true) {
            if (extension.ui.hover(extension.ui.drag)) {
                extension.action = 2;
            ...
            if (extension.ui.hover(extension.ui.resize)) {
                extension.action = 3;
        ...
        document.dispatchEvent(new CustomEvent('ui-mousedown', {
            detail: {
                event: event,
                clientX: event.clientX,
                clientY: event.clientY,
                x: event.clientX - extension.ui.offsetLeft - extension.ui.surface.offsetLeft,
                y: event.clientY - extension.ui.offsetTop - extension.ui.surface.offsetTop
            }
        }));
...
extension.events.mouse.contextmenu = function (event) {
    if (extension.ui.hover(extension.ui.surface)) {
        event.preventDefault();
        event.stopPropagation();

        document.dispatchEvent(new CustomEvent('ui-contextmenu', {
            detail: {
                x: event.clientX - extension.ui.offsetLeft - extension.ui.surface.offsetLeft,
                y: event.clientY - extension.ui.offsetTop - extension.ui.surface.offsetTop
            }
...
extension.events.mouse.mouseup = function (event) {
    ...
    if (extension.ui.hover(extension.ui.surface)) {
        ...
        document.dispatchEvent(new CustomEvent('ui-mouseup', {
            detail: {
                event: event,
                clientX: event.clientX,
                clientY: event.clientY,
...
extension.events.mouse.click = function (event) {
    if (extension.ui.hover(extension.ui.toggle)) {
        extension.ui.actions.toggle();
    }

    if (
        extension.ui.hover(extension.ui.toggle) ||
        (
            extension.storage.items.hidden !== true &&
            (
                extension.ui.hover(extension.ui.drag) ||
                extension.ui.hover(extension.ui.resize)
            )
        )
    ) {
        event.preventDefault();
...
extension.cursor.check = function (video) {
    ...
    if (
        ...
        document.dispatchEvent(new CustomEvent('cursor-check'));
...
extension.ui.focus = function (node) {
    if (node) {
        node.setAttribute('focus', '');

        extension.ui.focused = node;
    } else if (extension.ui.focused) {
        extension.ui.focused.removeAttribute('focus');

        delete extension.ui.focused;
    }
};
...
extension.ui.update = function () {
    ...
    if (video) {
        if (this.classList.contains(extension.prefix + '--busy') === false) {
            if (extension.ui.hover(this.surface)) {
                this.surface.classList.add(extension.prefix + '__surface--hover');

                if (extension.ui.hover(this.toggle)) {
                ...
                if (extension.storage.items.hidden !== true && extension.ui.hover(this.drag)) {
                ...
                if (extension.storage.items.hidden !== true && extension.ui.hover(this.resize)) {
...
extension.ui.hover = function (node) {
